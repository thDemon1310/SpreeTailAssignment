...
| 5 | Settlement logged as expense | All three: split_type blank + single recipient in split_with + description matches `\b(paid|repaid|returned|gave|sent|back|settled|settlement)\b`. Two-of-three = blocked. | Row 14 (Rohan paid Aisha back) | Auto-route to Settlement table (not Expense). Fallback: 2/3 conditions = blocked for manual review. | No — Settlement row created. Human cannot revert without deleting the Settlement row directly. |
...
| 17 | Expense is a deposit/transfer, not a shared cost | split_type non-blank + single recipient in split_with + description matches `\b(deposit|transfer|advance|moving in|moved in)\b`. Separate from settlement rule. | Row 38 (Sam deposit share) | Block row. No Expense written. Human must confirm and reclassify via UI. | Yes — human can reclassify via UI. |
